...
fn generate_twice_produces_identical_bytes() {
    ...
    assert_eq!(
        first.len(),
        42,
        "twenty-one registered roots produce twenty-one JSON Schema artifacts plus \
         twenty-one TypeScript counterparts; update this pin when the registry changes"
    );
}
...
fn generated_typescript_artifacts_mirror_the_schema_tree() {
    ...
    assert_eq!(
        typescript.len(),
        21,
        "twenty-one roots must yield twenty-one TypeScript files; the generator emits no \
         TypeScript yet"
    );
...
fn generated_typescript_is_byte_deterministic() {
    ...
    assert_eq!(
        ...
        21,
        "twenty-one roots must yield twenty-one TypeScript files"
    );
