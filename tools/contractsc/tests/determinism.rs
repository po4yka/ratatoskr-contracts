...
fn generate_twice_produces_identical_bytes() {
    ...
    assert_eq!(
        first.len(),
        98,
        "forty-nine registered roots produce forty-nine JSON Schema artifacts plus \
         forty-nine TypeScript counterparts; update this pin when the registry changes"
    );
}
...
fn generated_typescript_artifacts_mirror_the_schema_tree() {
    ...
    assert_eq!(
        typescript.len(),
        49,
        "forty-nine roots must yield forty-nine TypeScript files"
    );
