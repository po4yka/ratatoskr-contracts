...
fn generate_twice_produces_identical_bytes() {
    ...
    assert_eq!(
        first.len(),
        90,
        "forty-five registered roots produce forty-five JSON Schema artifacts plus \
         forty-five TypeScript counterparts; update this pin when the registry changes"
    );
}
...
fn generated_typescript_artifacts_mirror_the_schema_tree() {
    ...
    assert_eq!(
        typescript.len(),
        45,
        "forty-five roots must yield forty-five TypeScript files"
    );
