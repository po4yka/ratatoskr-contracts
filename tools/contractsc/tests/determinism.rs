...
fn generate_twice_produces_identical_bytes() {
    ...
    assert_eq!(
        first.len(),
        64,
        "thirty-two registered roots produce thirty-two JSON Schema artifacts plus \
         thirty-two TypeScript counterparts; update this pin when the registry changes"
    );
}
...
fn generated_typescript_artifacts_mirror_the_schema_tree() {
    ...
    assert_eq!(
        typescript.len(),
        32,
        "thirty-two roots must yield thirty-two TypeScript files"
    );
