...
fn generate_twice_produces_identical_bytes() {
    ...
    assert_eq!(
        first.len(),
        40,
        "twenty registered roots produce twenty JSON Schema artifacts plus twenty \
...
fn generated_typescript_artifacts_mirror_the_schema_tree() {
    ...
    assert_eq!(
        typescript.len(),
        20,
        "twenty roots must yield twenty TypeScript files; the generator emits no \
         TypeScript yet"
    );
...
fn generated_typescript_is_byte_deterministic() {
    ...
    assert_eq!(
        ...
        20,
        "twenty roots must yield twenty TypeScript files"
    );
