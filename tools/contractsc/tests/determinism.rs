...
fn generate_twice_produces_identical_bytes() {
    ...
    assert_eq!(
        first.len(),
        62,
        "thirty-one registered roots produce thirty-one JSON Schema artifacts plus \
         thirty-one TypeScript counterparts; update this pin when the registry changes"
    );
}
...
fn generated_typescript_artifacts_mirror_the_schema_tree() {
    ...
    assert_eq!(
        typescript.len(),
        31,
        "thirty-one roots must yield thirty-one TypeScript files"
    );
