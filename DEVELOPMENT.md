...
cargo test --workspace --locked
cargo contracts generate                     # must rewrite nothing
git diff --exit-code                         # the clean-checkout guarantee (ARCHITECTURE S11)
```

`cargo contracts generate` MUST NOT run before `cargo contracts check` in the gate. `generate` repairs
drift in place, so a `check` that follows it inspects a tree that was just repaired and its drift step
reports `current` for an artifact that was committed wrong. `cargo test` after it then validates
regenerated bytes rather than committed ones. Verified: tamper with any file under `schemas/`, and
`generate` then `check` exits 0 while `check` alone exits 1. In the gate, `generate` runs LAST and only
as a belt-and-braces proof of the write path, where rewriting nothing is the pass condition.
