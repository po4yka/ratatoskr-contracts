...
Both need the producer installed once: `cargo install --locked --version 0.52.0 cargo-public-api`. It drives rustdoc JSON through a nightly compiler and installs none itself — the machine needs `rustup toolchain install nightly-2026-06-12 --profile minimal` beside the pinned stable. Run either command with `RUSTUP_TOOLCHAIN=nightly-2026-06-12` so it uses the compiler recorded by the current baseline provenance. The version and the nightly date are both pinned because each shapes the emitted text (0.52.0 dropped function parameter names from the default output), so bumping either pin means rerunning `api-write` and committing the moved baselines with the upgrade; every baseline carries a provenance header naming what produced it. Neither verb is part of the gate — CI runs `api-check` in the `compatibility` job of `.github/workflows/contracts.yml`.
