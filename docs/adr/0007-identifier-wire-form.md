...
5. One identity has one spelling. The repository already applies this to instants, to `TenantRef`, to `BlobRef` and to every typed UUID.
...
- `EntityRef` accepts `blob:sha256:<UPPERCASE-HEX>` while `BlobRef` rejects it. This is **not** treated, and the non-treatment is deliberate: no `BlobRef` ↔ `EntityRef` conversion exists anywhere, `OperationResultRef.target` and `OperationResultRef.blob` are independent fields, and so no join can miss. The UUID rule exists precisely because `EventId`/`OperationId`/`UserId`/`CorrelationId::try_from(&EntityRef)` do exist and route through `as_uuid()`. If a `BlobRef` bridge is ever added, this rule extends to it in the same commit.
...
- **`ErrorEnvelope.trace_id` is a bare pointer.** It points at a distributed trace and is 32 bare hex characters. Under a two-clause is-a/points-at rule it would have to become `trace:4bf9…`, which would break W3C Trace Context interoperability. This is why clause 3 exists and why the rule is three clauses, not two. Clause 3 is not a carve-out invented to save the rule: `TraceId` and `BlobRef` are what already ship.
...
- If a `BlobRef` ↔ `EntityRef` conversion is ever added, extend the canonicality rule to the `sha256:<hex>` local form in the same commit.
