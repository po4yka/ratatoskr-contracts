//! Compatibility fixtures — test C-4. `ARCHITECTURE.md` S12 requires both directions per family.
...
use ratatoskr_operation_contracts::OperationSnapshot;
...
fn bucket(direction: &str) -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../..")
        .join("fixtures/platform/operation-snapshot/compat")
        .join(direction)
}
...
fn accepts_losslessly(path: &Path) {
    ...
    let decoded: OperationSnapshot = serde_json::from_str(&bytes)
...
/// C-4, forward half: today's type accepts and re-emits what a newer producer sent.
#[test]
...
/// C-4, backward half: a payload frozen at the first shape still parses today.
#[test]
